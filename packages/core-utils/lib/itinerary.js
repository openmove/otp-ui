...
function getStepDirection(step) {
  switch (step.relativeDirection) {
    case "DEPART":
      return `Vai verso ${convertAbsoluteDirection(step.absoluteDirection.toLowerCase())}`;
    ...
    case "CIRCLE_CLOCKWISE":
      return "Prendi la rotonda in senso orario";

    case "CIRCLE_COUNTERCLOCKWISE":
      return "Prendi la rotonda in senso antiorario";

    case "ELEVATOR":
      return "Prendi l'ascensore";

    case "UTURN_LEFT":
      return "Inversione a U a sinistra";

    case "UTURN_RIGHT":
      return "Inversione a U a destra";
...
function getStepInstructions(step) {
  const conjunction = step.relativeDirection === "ELEVATOR" ? "a" : "su";
  return `${getStepDirection(step)} ${conjunction} ${getStepStreetName(step)}`;
}

function getStepStreetName(step) {
  if (step.streetName === "road") return "Strada senza nome";
  if (step.streetName === "path") return "Via senza nome";
...
function getLegModeLabel(leg) {
  switch (leg.mode) {
    ...
    case "CAR":
      return leg.hailedCar ? "Guida" : "Guida";
